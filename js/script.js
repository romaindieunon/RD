...
/* ---------- Formulaire de contact (ouvre Gmail compose dans un nouvel onglet) ---------- */
...
if (form) {
    form.addEventListener('submit', (e) => {
        ...
        const motif      = (form.querySelector('#motif')?.value      || '').trim();
        const message    = (form.querySelector('#message')?.value    || '').trim();

        if (!prenom || !nom || !email || !motif || !message) {
            if (status) {
                status.textContent = 'Merci de remplir tous les champs obligatoires avant d\u2019envoyer.';
        ...
        const sujet  = `[Portfolio] ${motif} \u2014 ${prenom} ${nom}`;
        let corps =
            `Bonjour Romain,\n\n` +
            `${message}\n\n` +
            `\u2014\n` +
        ...
        // URL Gmail compose : ouvre la r\u00e9daction Gmail dans le navigateur (pas besoin de client mail install\u00e9)
        const gmailUrl =
        ...
        // Ouvre dans un nouvel onglet pour ne pas perdre le portfolio
        const win = window.open(gmailUrl, '_blank', 'noopener,noreferrer');

        if (status) {
            if (win) {
                status.textContent = 'Gmail s\u2019ouvre dans un nouvel onglet avec votre message pr\u00e9-rempli. Cliquez sur Envoyer pour le finaliser.';
                status.style.color = 'var(--success)';
            } else {
                // Si le popup a \u00e9t\u00e9 bloqu\u00e9, on propose de cliquer manuellement
                status.innerHTML = 'Votre navigateur a bloqu\u00e9 l\u2019ouverture automatique. ' +
                    '<a href="' + gmailUrl + '" target="_blank" rel="noopener" style="color: var(--primary); text-decoration: underline;">Cliquez ici pour ouvrir Gmail</a>.';
                status.style.color = '#dc2626';
            }
        }
    });
